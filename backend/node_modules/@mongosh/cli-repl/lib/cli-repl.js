...
class CliRepl {
    constructor(options) {
        ...
        this.promptOutput = (_a = options.promptOutput) !== null && _a !== void 0 ? _a : options.output;
        ...
        this.globalConfigPaths = (_b = options.globalConfigPaths) !== null && _b !== void 0 ? _b : [];
        ...
        this.logManager = new mongodb_log_writer_1.MongoLogManager({
            ...
            retentionDays: 30,
            maxLogFileCount: +(process.env.MONGOSH_TEST_ONLY_MAX_LOG_FILE_COUNT || 100),
            onerror: (err) => this.bus.emit('mongosh:error', err, 'log'),
            onwarn: (err, path) => this.warnAboutInaccessibleFile(err, path),
        });
        this.agent = (0, devtools_proxy_support_1.useOrCreateAgent)(this.proxyOptions);
        ...
        this.mongoshRepl = new mongosh_repl_1.default({
            ...
            nodeReplOptions: (_c = options.nodeReplOptions) !== null && _c !== void 0 ? _c : {
    ...
    async start(driverUri, driverOptions) {
        ...
        if (!this.cliOptions.nodb) {
            ...
            if (await this.isTlsKeyFilePasswordMissingURI(searchParams)) {
                const keyFilePassword = encodeURIComponent(await this.requirePassword('Enter TLS key file password'));
    ...
    async connect(driverUri, driverOptions) {
        ...
        return await service_provider_node_driver_1.NodeDriverServiceProvider.connect(driverUri, driverOptions, this.cliOptions, this.bus);
    }
    getHistoryFilePath() {
    ...
    async isTlsKeyFilePasswordMissingURI(searchParams) {
        ...
        if (tlsCertificateKeyFile && !tlsCertificateKeyFilePassword) {
            ...
            return (contents.search(/(ENCRYPTED PRIVATE KEY|Proc-Type: 4,ENCRYPTED)/) !== -1);
        }
        return false;
    }
    ...
    async requirePassword(passwordPrompt = 'Enter password') {
    ...
    async close() {
        ...
        return ((_a = this.closingPromise) !== null && _a !== void 0 ? _a : (this.closingPromise = (async () => {
            ...
            (_b = this.logWriter) === null || _b === void 0 ? void 0 : _b.info('MONGOSH', (0, mongodb_log_writer_1.mongoLogId)(1000000045), 'analytics', 'Flushed outstanding data', {
                flushError,
                flushDuration,
            });
            await ((_c = this.logWriter) === null || _c === void 0 ? void 0 : _c.flush());
    ...
    async prepareOIDCOptions(driverUri, driverOptionsIn) {
        ...
        (_a = (_j = driverOptions.oidc).allowedFlows) !== null && _a !== void 0 ? _a : (_j.allowedFlows = ['auth-code']);
        (_b = (_k = driverOptions.oidc).notifyDeviceFlow) !== null && _b !== void 0 ? _b : (_k.notifyDeviceFlow = ({ verificationUrl, userCode }) => {
            this.output.write('\n' +
        ...
        (_c = driverOptions.proxy) !== null && _c !== void 0 ? _c : (driverOptions.proxy = this.proxyOptions);
        (_d = driverOptions.applyProxyToOIDC) !== null && _d !== void 0 ? _d : (driverOptions.applyProxyToOIDC = true);
        ...
        if (redirectURI !== undefined) {
            (_e = (_l = driverOptions.oidc).redirectURI) !== null && _e !== void 0 ? _e : (_l.redirectURI = redirectURI);
        }
        if (browser !== undefined) {
            (_f = (_m = driverOptions.oidc).openBrowser) !== null && _f !== void 0 ? _f : (_m.openBrowser = browser !== false ? { command: browser } : browser);
        }
        if (trustedEndpoints !== undefined) {
            (_g = (_o = driverOptions.authMechanismProperties).ALLOWED_HOSTS) !== null && _g !== void 0 ? _g : (_o.ALLOWED_HOSTS = trustedEndpoints);
        }
        if (process.env.MONGOSH_OIDC_PARENT_HANDLE) {
            (_h = driverOptions.parentHandle) !== null && _h !== void 0 ? _h : (driverOptions.parentHandle = process.env.MONGOSH_OIDC_PARENT_HANDLE);
        }
    ...
    setupOIDCTokenDumpListener() {
        function tryParseJWT(token, redact) {
            if (!token)
                return token;
            const jwtParts = token.split('.');
            if (jwtParts.length === 3 &&
                jwtParts.every((part) => Buffer.from(part, 'base64url')
                    .toString('base64url')
        ...
        let lastServerIdPInfo;
        const { oidcDumpTokens } = this.cliOptions;
        if (oidcDumpTokens) {
            this.bus.on('mongodb-oidc-plugin:received-server-params', ({ params: { idpInfo } }) => {
            ...
            this.bus.on('mongodb-oidc-plugin:auth-succeeded', ({ tokenType, refreshToken, expiresAt, passIdTokenAsAccessToken, tokens: { accessToken: at, refreshToken: rt, idToken: idt }, }) => {
                const printable = {
                    lastServerIdPInfo: lastServerIdPInfo && {
                        issuer: lastServerIdPInfo === null || lastServerIdPInfo === void 0 ? void 0 : lastServerIdPInfo.issuer,
                        clientId: lastServerIdPInfo === null || lastServerIdPInfo === void 0 ? void 0 : lastServerIdPInfo.clientId,
                        requestScopes: lastServerIdPInfo === null || lastServerIdPInfo === void 0 ? void 0 : lastServerIdPInfo.requestScopes,
                    },
                    tokenType,
                    refreshToken,
                    expiresAt,
                    passIdTokenAsAccessToken,
                    tokens: oidcDumpTokens === 'include-secrets'
                        ? {
