...
function setupLoggerAndTelemetry(bus, log, analytics, userTraits, mongosh_version) {
    ...
    const getTelemetryUserIdentity = () => ({
        anonymousId: telemetryAnonymousId !== null && telemetryAnonymousId !== void 0 ? telemetryAnonymousId : userId,
    });
    ...
    bus.on('mongosh:write-custom-log', (event) => {
        log[event.method]('MONGOSH-SCRIPTS', (0, mongodb_log_writer_1.mongoLogId)(1000000054), 'custom-log', event.message, event.attr, event.level);
    });
    bus.on('mongosh:connect', function (args) {
        ...
        log.info('MONGOSH', (0, mongodb_log_writer_1.mongoLogId)(1000000004), 'connect', 'Connecting to server', {
            userId,
    ...
    bus.on('mongosh:error', function (error, context) {
        ...
        if (context === 'fatal') {
            log.fatal('MONGOSH', (0, mongodb_log_writer_1.mongoLogId)(1000000006), context, `${mongoshError.name}: ${mongoshError.message}`, error);
        }
        else {
            log.error('MONGOSH', (0, mongodb_log_writer_1.mongoLogId)(1000000006), context, `${mongoshError.name}: ${mongoshError.message}`, error);
        }
    ...
    bus.on('mongosh:fetching-update-metadata', function (ev) {
        log.info('MONGOSH', (0, mongodb_log_writer_1.mongoLogId)(1000000052), 'startup', 'Fetching update metadata', {
            ...ev,
        });
    });
    bus.on('mongosh:fetching-update-metadata-complete', function (ev) {
        log.info('MONGOSH', (0, mongodb_log_writer_1.mongoLogId)(1000000053), 'startup', 'Fetching update metadata complete', {
            ...ev,
        });
    });
}
